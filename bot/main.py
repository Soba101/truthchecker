...
class TruthWarsBot:
    ...
    async def setup_bot_commands(self) -> None:
        ...
        try:
            ...
            logger.info("=" * 70)
            logger.info("🚨 CRITICAL: BOTFATHER CONFIGURATION REQUIRED 🚨")
            logger.info("Commands will show @username suffix until BotFather is configured!")
            logger.info("")
            logger.info("1. Message @BotFather on Telegram")
            logger.info("2. Send: /setcommands")
            logger.info("3. Select your bot")
            logger.info("4. Copy and paste EXACTLY this list:")
            logger.info("")
            for cmd in commands:
                logger.info(f"{cmd.command} - {cmd.description}")
            logger.info("")
            logger.info("5. Send: /setprivacy")
            logger.info("6. Select your bot") 
            logger.info("7. Choose: Disable")
            logger.info("")
            logger.info("8. Restart this bot after completing steps above")
            logger.info("=" * 70)
